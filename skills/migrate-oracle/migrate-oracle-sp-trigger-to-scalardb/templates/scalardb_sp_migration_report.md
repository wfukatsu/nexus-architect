# Oracle Stored Procedure Migration Report
...
| Packages | {{PKG_COUNT}} | {{PKG_LOW}} | {{PKG_MED}} | {{PKG_HIGH}} |
...
- **Total PL/SQL Objects**: {{TOTAL_COUNT}}
...
| # | Package Name | Procedures | Functions | Complexity | Generated File |
|---|-------------|------------|-----------|------------|----------------|
| {{N}} | {{PKG_NAME}} | {{PROC_COUNT}} | {{FUNC_COUNT}} | {{COMPLEXITY}} | `{{FILENAME}}` |

### Triggers

| # | Trigger Name | Table | Type | Event | Complexity | Generated File |
|---|-------------|-------|------|-------|------------|----------------|
| {{N}} | {{TRIGGER_NAME}} | {{TABLE}} | {{TRIGGER_TYPE}} | {{EVENT}} | {{COMPLEXITY}} | `{{FILENAME}}` |

---

## 3. Feature Usage Matrix

Shows which of the 17 feature categories each PL/SQL object uses:
...
| 2 | Cursors (DECLARE CURSOR/FETCH) | `tx.scan()` + iteration |
| 3 | Control flow (IF/WHILE/CASE) | Plain Java |
| 4 | Exception handling (TRY/CATCH) | ScalarDB exception types |
...
| 12 | Sequences/AUTO_INCREMENT | UUID or counter table |
| 13 | Temp tables | Java collections |
| 14 | Dynamic SQL | Builder pattern |
...
- [ ] Validate data type precision (especially NUMBER → BIGINT/DOUBLE conversions)
