...
3. If `reports/before/{project}/*.md` exists, add it as input — including the UI analysis when
   `/architect:analyze-ui` ran: each feature in `ui-features.md` is a candidate FR, recorded with its
   `UIF-` node as the FR's `upstream` in `work/traceability.json`; the validation rules it lists are
   acceptance-criteria candidates, and a rule enforced only in the browser is flagged as such, never
   adopted as a rule the system enforces
4. **Auto-detect product output.** Glob `reports/00_core/`, `reports/01_ux/`, `reports/02_spec/`, `reports/03_domain/`, `reports/04_quality/` and `work/traceability.json`; add any present as inputs. Match **files**, not directories — `/product:init-output` creates `reports/01_ux/domain-stories/` and `reports/02_spec/ui-mocks/` empty, so a directory test would report a handoff on a product project where no phase ever ran. State in the output which product artifacts were found and which were absent: a partial product run (e.g. `--profile=mvp` stops before `feature-list.md`/`nfr.md`) changes what can be carried over, and silently proceeding hides that no `FEAT-`→`FR-` link exists. Map them per @docs/design.md §1.3 — in particular: `nfr.md`/`sla.md` → NFR table (reuse `NFR-` IDs verbatim), `feature-list.md` → FR (`FEAT-`→`FR-`, record the link; when `examples/example-map-*.md` exists, the feature's `RULE-` entries are the FR's acceptance criteria, cited by ID), `bounded-contexts.md`/`ubiquitous-language.md` → context scoping, `scope-definition.md`/`constraints.md` → scope, `assumptions.md` → open questions, `tech-stack-fitness.md` → prior for the ScalarDB applicability verdict (Step 4). Treat the §1.4 designed gaps (per-process transaction consistency, physical DB inventory, actor/role/permission) as items product does *not* supply — these drive elicitation.
