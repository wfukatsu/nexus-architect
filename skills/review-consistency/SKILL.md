...
- Bounded Context Canvases: every context in `bounded-contexts-redesign.md` has all nine Canvas parts; when the product-side `reports/03_domain/bounded-contexts.md` exists, each `CTX-` keeps its id across the handoff, and a changed purpose, classification or inbound/outbound set is recorded as a decision, not silently redrawn
