# Subagent Prompt: MySQL Stored Procedure & Trigger Migration

**Type:** general-purpose
**Description:** Reads MySQL routine definitions from raw_mysql_schema_data.json and generates equivalent ScalarDB Java application code (Transaction API only) for stored procedures, functions, and triggers, with a summary migration report.
...
7. For each procedure, function, and trigger found in the JSON:
...
8. Write the summary report to: <OUTPUT_DIR>/scalardb_mysql_sp_migration_report.md using the template
9. Record end time and compute duration using Bash: `END_SECS=$(date +%s) && echo $((END_SECS - START_SECS))`

**Important rules:**
- If the `routines` section has no procedures, functions, or triggers, report STATUS: SUCCESS with 0 files generated
...
- Use the MYSQL_DATABASE (lowercase) as the ScalarDB namespace
...
- REPORT_FILE: <OUTPUT_DIR>/scalardb_mysql_sp_migration_report.md
