# Subagent Prompt: PostgreSQL Schema Report Generation

**Type:** general-purpose
**Description:** Reads raw_schema_data.json and the report template, then generates postgresql_schema_report.md with a complete human-readable schema analysis.

## Prompt

You are generating a PostgreSQL schema analysis report. Follow these steps:

0. Record start time using Bash: `START_SECS=$(date +%s)`
1. Read the skill instructions at: <PLUGIN_ROOT>/skills/migrate-postgresql/analyze-postgresql-schema/SKILL.md
2. Read the report template at: <PLUGIN_ROOT>/skills/migrate-postgresql/analyze-postgresql-schema/analyze-postgresql-dbms_report.md
...
5. Write the report to: <OUTPUT_DIR>/postgresql_schema_report.md
...
- REPORT_FILE: <OUTPUT_DIR>/postgresql_schema_report.md
- DURATION_SECONDS: <elapsed seconds computed in step 6>
- SUMMARY: <executive summary — table count, view count, function count, custom type count, total objects, key observations in 2-3 lines>
