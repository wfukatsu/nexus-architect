...
### Added
- **`/architect:analyze-ui` — 既存 UI を証拠として扱う。** これまでのレガシー経路はバックエンドしか読まず、
  画面を棚卸しするスキルがなかったため、数百の JSP を持つシステムでも画面、その入出力、UI だけのロール
  チェック、スクリプトレットや jQuery に埋もれた業務ルールが見えないまま再設計に進んでいた。新しい任意工程は
  `investigate` の後・`analyze` の前に走り、テンプレートより先にルートマップを読み、画面を並列バッチで抽出して
  正本のインベントリ（`ui-inventory.json`）を書く。内容は、全画面の INPUT（項目、コントロール、検証とそれが
  クライアント・サーバー・両方のどこで効くか）、OUTPUT、操作（遷移先と送信する入力）、画面単位と項目単位の
  ロールガード、画面層のロジック、言語、画像、色の組。全部品のバリアント・状態・使用箇所・手作りの重複。
  同じコマンドを持つ操作の集まりとしての全機能。そして効率を測る単位となる、ユーザーが歩く**タスク**。
  as-is の W3C DTCG トークンファイルは近い値を統合せず断片化を見せ、`/product:design-system --import` が
  そのまま取り込める。4 つのビュー（遷移図と画面 → コード対応を含む画面カタログ、機能とタスク、部品、
  デザインシステム抽出）は `tools/lib/ui_views.py` が描画し、手で書き直さない。`UIS-`/`UIC-`/`UIF-` を
  接頭辞として登録し、`rules/ui-analysis.md` が数え方、UI 技術別の検出、形と 10 の整形式ルールを定め、
  `tools/lib/ui_inventory.py` がそれを強制する（引用したソース行が解析対象のコードに実在することも含む）。
- **`/architect:evaluate-ux` — 判断に柵を設けた UX 指数。** 5 軸（Nielsen ヒューリスティック 30%、WCAG 2.2
  アクセシビリティ 25%、タスク効率 20%、一貫性 15%、ナビゲーション 10%）を半開区間の帯を持つ UXI にまとめる。
  件数・比率・コントラスト値はすべて `tools/lib/ui_metrics.py` が算出する。所見は固定の欠陥語彙から 1 つを
  名指しし、それが軸・基準・既定の重大度を決めるため、1 つの欠陥は 1 回しか起票されない。各軸のスコアは
  計量値による上限（3 未満には下げない）と自軸の所見の重大度（critical があれば 2 以下）の両方で縛られる。
  5 つのサブエージェントが結果をファイルに書き、親が統合して指数を計算する。`tools/lib/ux_evaluation.py` は、
  計量値が再計算と異なる、スコアがどちらかの柵を越える、所在が食い違う、実画面の証拠がその所見の画面の
  キャプチャでない、のいずれかで評価を不合格にする。計量値が測った欠陥は必ず、それもその場所にだけ起票し、
  親は採点前に各所見を出典と照合する。security の事実と画面層のロジックは採点せず、回付一覧
  （`ux_evaluation.py --routed`）に集める。既定は静的な専門家レビューでその旨を明記し、
  `--base-url` で画面ごとのスクリーンショット、アクセシビリティ検査、ターゲットサイズ、送信後のエラー状態を
  加える。ロール別のセッションは実行後に削除する。
- **接続。** `investigate` は UI 工程を走らせるかを決める Presentation Layer 節を出す。`analyze` はロール
  ガード・ラベル・画面をアクター表、ユビキタス言語、ドメイン-コード対応の画面列の証拠にする。
  `integrate-evaluations` は UX の系統を加え、`define-requirements` はレガシーの `FR-` の上流に `UIF-` を
  記録する。`start` は `investigate` の後で 2 工程を提案し、`pipeline` は `--auto` で静的に走らせるか、UI が
  無ければ skipped を記録する。`samples/legacy-ui-shop/` は UI 欠陥 21 件、その答え合わせ表、
  そして参照セット（`expected-reports/`）を持つ起動できる JSP + jQuery のショップ。参照セットのテストは、
  仕込んだ欠陥がすべて見つかることと、ビューが `ui_views.py` の描画結果と一致することを要求する。

## [0.38.2] - 2026-09-10

### Fixed
- **`/product:report` が Mermaid 図をすべて構文エラーにしたレポートを出さなくなった。** スキルは
  HTML の「何を」だけを述べてツールを名指ししていなかったため、実走行では pandoc で Markdown を変換し、
  Mermaid フェンスが `<pre class="mermaid"><code>…</code></pre>` になっていた。Mermaid の
  ...
  `<pre class="mermaid">` を走査し、`<code>` ラッパーを重大エラーとして扱う。
