...
- **`/architect:analyze-ui` — 既存 UI を証拠として扱う。** これまでのレガシー経路はバックエンドしか読まず、
  ...
  `investigate` の後・`analyze` の前に走り、テンプレートより先にルートマップを読み、正本のインベントリ
  （`ui-inventory.json`: 全画面の INPUT — 項目、コントロール、検証とそれがクライアント・サーバー・両方の
  どこで効くか — OUTPUT、操作と遷移、アクセスガード、画面層のロジック、言語、画像、色の組。全部品の
  バリアント・状態・使用箇所・手作りの重複。同じコマンドを持つ submit/AJAX 操作の集まりとしての全機能）と、
  近い値を統合せず断片化が見える as-is の W3C DTCG トークンファイル、そして 4 つのビュー（遷移図と画面 →
  コード対応を含む画面カタログ、機能、部品、`/product:design-system --import` がそのまま取り込める
  デザインシステム抽出）を書く。`UIS-`/`UIC-`/`UIF-` を接頭辞として登録し、`rules/ui-analysis.md` が
  数え方、UI 技術別の検出、形と 10 の整形式ルールを定め、`tools/lib/ui_inventory.py` がそれを強制する
  （引用したソース行が解析対象のコードに実在することも含む）。
- **`/architect:evaluate-ux` — 測っていない数値を載せられない UX 指数。** 5 軸（Nielsen ヒューリスティック
  30%、WCAG 2.2 アクセシビリティ 25%、タスク効率 20%、一貫性 15%、ナビゲーション 10%）を半開区間の帯を持つ
  UXI にまとめる。件数・比率・コントラスト値はすべて `tools/lib/ui_metrics.py` が算出し、計量値から各軸の
  上限も決める。5 つのサブエージェントはスコアと所見だけを返し、指数は親が計算する。
  `tools/lib/ux_evaluation.py` は、計量値が再計算と異なる、スコアが上限を超える、UXI が式の値でない、
  所見が存在しない画面・部品・トークンを指す、のいずれかで評価を不合格にする。既定は静的な専門家レビューで
  その旨を明記し、`--base-url` で画面ごとのスクリーンショットと描画後のアクセシビリティ検査を加える。
- **接続。** `investigate` は UI 工程を走らせるかを決める Presentation Layer 節を出す。`analyze` はロール
  ...
  無ければ skipped を記録する。`samples/legacy-ui-shop/` は UI 欠陥 21 件とその答え合わせ表を持つ、起動
  できる JSP + jQuery のショップ。
