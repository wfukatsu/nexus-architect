...
| **Transactions are boundaries, not collaborators** | The application service (the interactor, under the `clean` layering style of `design-implementation`) opens the ScalarDB transaction and passes it down; domain objects never hold or start one. This is what lets the Fake stand in — the domain does not know a transaction exists (@rules/scalardb-coding-patterns.md) |
