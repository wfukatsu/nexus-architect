...
| `tools/lib/aggregate_manifest.test.py` | The aggregate design contract: the seven well-formedness rules of `rules/aggregate-design.md` §3 (one root, at least one stated invariant, every invariant violable by a declared command, actor / consistency class / emitted event per command, interior reachable only through the root, other aggregates referenced by identity) plus a positive and a negative concrete example per invariant, the `STM-` link resolving against the state-machine manifest, and malformed shapes reported rather than crashing |
