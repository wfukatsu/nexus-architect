...
| `tools/lib/ux_evaluation.test.py` | The UX evaluation contract: the nine rules of `rules/ux-evaluation.md` §6 — five axes with the rule's weights (and the rule's table stating the same weights), the UXI equal to the formula, the half-open band, complete findings citing H1–H10 or a WCAG criterion, locations that exist, low scores justified, the metrics block equal to what `ui_metrics.py` recomputes, no score above its cap, runtime evidence that exists |
