...
| `tools/lib/aggregate_manifest.test.py` | The aggregate design contract: the seven well-formedness rules of `rules/aggregate-design.md` §3 (one root, at least one stated invariant, every invariant violable by a declared command, actor / consistency class / emitted event per command, interior reachable only through the root, other aggregates referenced by identity) plus a positive and a negative concrete example per invariant, the `STM-` link resolving against the state-machine manifest, `also_writes` naming only other declared aggregates, one publishing aggregate per event, and malformed shapes reported rather than crashing |
| `tools/lib/ui_inventory.test.py` | The existing-UI inventory contract: the ten well-formedness rules of `rules/ui-analysis.md` §4 (unique `UIS-`/`UIC-`/`UIF-`, action ids and task names; every screen named, sourced and roled, with an entry screen; typed inputs and accessibility facts; every action resolving to a declared screen, an endpoint or a stated reason, with the inputs it sends; component `used_by` agreeing with the screens both ways; every submit/AJAX action carrying a command owned by exactly one feature; classified view-layer logic; every source line existing in the analysed code; a valid DTCG token file; every feature used in a task whose path follows declared transitions) and hostile shapes reported rather than crashing |
| `tools/lib/ui_metrics.test.py` | The UX metrics `/architect:evaluate-ux` may cite: WCAG contrast ratios, the navigation graph of `rules/ui-analysis.md` §6 (depth, orphans, unreachable screens, dead ends), input burden, token fragmentation and label drift, and the per-axis score caps of `rules/ux-evaluation.md` §2 |
| `tools/lib/ux_evaluation.test.py` | The UX evaluation contract: the ten rules of `rules/ux-evaluation.md` §6 — five axes with the rule's weights, every score within its metric cap and within the bound its own findings' severity sets, the UXI equal to the formula, the half-open band, every finding a defect from the rule's vocabulary on the axis that owns it with a criterion that axis owns and a reason for any non-default severity, locations that exist and agree (action and input of the screen, component used by it), each finding listed by its own axis, the metrics block equal to what `ui_metrics.py` recomputes, one finding per defect and subject, runtime evidence that is a capture of the finding's screen, every measured defect filed where the metrics show it, and the routed list complete — plus the rule's own tables (weights, defect vocabulary, criterion ownership) stating what the validator enforces |
| `tools/lib/ui_views.test.py` | The four `analyze-ui` views as projections of the inventory: both output hooks pass and no `#` heading in either language, the canonical section names present (English, or Japanese with the English in parentheses), metric-covered figures taken from the metrics, the facts a reader looks for on the page (validation and where it runs, where an action lands, dead ends and orphans styled, view-only guards marked, client-only rules flagged, consolidation proposals), byte-identical re-renders, and a malformed inventory refused |
...
| `tools/lib/domain_event_catalog.test.py` | The Domain Event Catalog (Published Language) contract stated in `skills/design-aggregate/SKILL.md` § Domain Event Catalog: one publisher per event and that publisher declaring it in the aggregate manifest, every declared event catalogued, consumers that are declared contexts other than the publisher's with a context-map relationship and a purpose, scope decided by the consumers, and a delivery contract (guarantee, idempotency key, version, evolution) on every published event |
...
**110 slash commands across four plugins.** The catalogue — every command with its model, its
...
group does what*, so you know where to look, and the counts below are a partition of all 110.
