...
| `tools/lib/ui_inventory.test.py` | The existing-UI inventory contract: the nine well-formedness rules of `rules/ui-analysis.md` §4 (unique `UIS-`/`UIC-`/`UIF-` and action ids, every screen named and sourced, typed inputs and accessibility facts, every action resolving to a declared screen or an endpoint or a stated reason, component `used_by` agreeing with the screens both ways, every submit/AJAX action owned by exactly one feature, classified view-layer logic, every source line existing in the analysed code, a valid DTCG token file) and hostile shapes reported rather than crashing |
