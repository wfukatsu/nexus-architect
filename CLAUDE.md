...
- `/product:report-status [--once] [--phase=<name>] [--exec] [--json] [--md] [--ascii] [--lang=ja|en]` — Show where the product pipeline stands on the terminal: the phase tree grouped by pipeline stage, each phase's status and declared-output completion, the validation gate's verdict and open-assumption count, per-phase cost, and a next-command action menu; the same `tools/nexus-status.sh` dashboard as `/architect:report-status` (pass `--once` for an in-session render)
